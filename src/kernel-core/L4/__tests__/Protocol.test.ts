...
describe('L4 Protocol System', () => {
    ...
    test('should trigger protocol when condition met', async () => {
        ...
        engine.ratify('p-cool', 'TRUSTED');
        engine.activate('p-cool');

        // Case 1: Temp = 20 (No Trigger)
        await kernel.execute(ActionFactory.create('temp', 20, admin.id, adminKeys.privateKey, '0:1'));
        await kernel.execute(ActionFactory.create('fan', 0, admin.id, adminKeys.privateKey, '0:2'));
        expect(state.get('fan')).toBe(0);

        // Case 2: Temp = 35 (Trigger)
        await kernel.execute(ActionFactory.create('temp', 35, admin.id, adminKeys.privateKey, '0:3'));
    ...
    test('should execute multiple rules', async () => {
        ...
        await kernel.execute(ActionFactory.create('fan', 10, admin.id, adminKeys.privateKey, '0:4'));
        await kernel.execute(ActionFactory.create('temp', 40, admin.id, adminKeys.privateKey, '0:5'));
